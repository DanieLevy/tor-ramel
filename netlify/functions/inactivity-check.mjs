...
if (vapidPublicKey && vapidPrivateKey) {
  try {
    ...
    console.log('✅ [Inactivity] VAPID keys configured successfully')
  } catch (error) {
    console.error('❌ [Inactivity] Failed to configure VAPID keys:', error)
  }
} else {
  console.warn('⚠️ [Inactivity] VAPID keys missing - push notifications will not work')
}
  ...
  try {
    ...
    for (const sub of pushSubs) {
      try {
        ...
        console.error(`[Inactivity] Push failed:`, pushError.message)
        failed++
    ...
    console.error(`[Inactivity] Error sending push to user ${userId}:`, error)
